...
const CalendarHeader = ({
	...
	useEffect(() => {
		const lookupTimetableData = async (
			urlParams: URLSearchParams,
		): Promise<void> => {
			const shareCode = urlParams.get('sid');
			if (!shareCode) return;
			const sharedLink = await lookupShareLink(shareCode, true);
			if (sharedLink.found) {
				const decoded = decodeTimetableFromBase64(sharedLink.data!);
...
const EndActions = () => {
	...
	const handleShare = async () => {
		try {
			const shareUrl = await generateShareURL(coursesData);
			if (!shareUrl) throw new Error();
