...
const API_BASE = import.meta.env.VITE_API_BASE_BACKEND || '';
...
export const generateShareURL = async (
	...
	try {
		const response = await fetch(`${API_BASE}/api/share-link/create`, {
			method: 'POST',
			headers: {
		...
		if (result.success) {
			...
			url.searchParams.set('sid', result.code);
			return url.toString();
		}
	} catch (error) {
...
): Promise<{ found: boolean; data: string | null }> => {
	try {
		const response = await fetch(`${API_BASE}/api/share-link/find/${code}`, {
			method: 'GET',
		});
		const result = await response.json();
		if (result.found) {
			return { found: true, data: returnData ? result.data : null };
		}
	} catch (error) {
		console.error('Failed to lookup share link:', error);
	}
